...
'window_height' = 400,
'playlist_ext': '.lpl',
'playlist_default': '~/.ListenPad/default.lpl',
'repo_path': '~/.ListenPad/repo',
'support_exts':['.mp3', '.ape', '.flac', '.ogg', '.wma', '.lpl', '.wav'],
}
...
app.data.state = ['idle', 'play', 'stop', 'pause']


app.commands = [
list.load, list.save, list.clear, list.add_dir, list.add_file
player.stop
player.pause
player.start
player.next
...
class Player:
    ...
    def __init__(self, proxy):
        ...
        self.timer.enable
        self.timer.value
        #self.current.meta = #[ar, ti, bps, length..., lyric], got from playlist[id]
        #self.current.lyric
        self.current.id
        self.watch(self.current.id, self.id_changed)

        self.playlist = [.....]

        # 必须定义触发动作
        self.state = [stop -- play, play -- pause, pause -> stop]
        self.watch(self.state, self.state_machine)
        ...
        def state_machine(old, new):
            pass



        self.commands = [self.next, self.play, self.stop, self.pause]
    ...
    def value_changed(state, watcher):
